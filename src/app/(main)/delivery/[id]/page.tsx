...
export default function Detail() {
    ...
    const progress = 50;
    return (
        <div className="min-h-screen-w-full m-10 items-center flex flex-col space-y-7">
            <SearchBar />
...
function NeedHelp() {
    return (
        <Card className="w-full flex flex-col py-6 px-8 bg-white border border-gray-200 shadow-lg rounded-lg">
            <p className='text-amber-600 text-2xl lg:text-xl md:text-lg sm:text-lg font-semibold mb-4'>Need help?</p>
            <div className="flex flex-col space-y-3">
                <Link href="/issues">
                    <p className="flex flex-row items-center space-x-2 text-gray-800 hover:text-amber-600 transition-colors text-base md:text-sm sm:text-sx cursor-pointer">
    ...
    return (
        <div className="w-full px-4 py-2">
            ...
            <div className="relative w-full h-2 bg-gray-300 rounded-full">
                ...
                {/* Milestones */}
                <div className="absolute flex justify-between w-full top-[-1.5rem]">
                    <div className="flex flex-col items-center">
                        <span className={`text-xs ${clampedProgress >= 0 ? 'text-amber-500' : 'text-gray-500'} mt-1`}>Order Confirmed</span>
                        <div className={`w-full h-4 bg-amber-500 rounded-full border ${clampedProgress >= 0 ? 'bg-amber-500' : 'bg-gray-500'}`}></div>
                    </div>
                    <div className="flex flex-col items-center">
                        <span className={`text-xs ${clampedProgress >= 50 ? 'text-amber-500' : 'text-gray-500'} mt-1`}>Delivery</span>
                        <div className={`w-full h-4 bg-amber-500 rounded-full border ${clampedProgress >= 50 ? 'bg-amber-500' : 'bg-gray-500'}`}></div>
                    </div>
                    <div className="flex flex-col items-center">
                        <span className={`text-xs ${clampedProgress >= 100 ? 'text-amber-500' : 'text-gray-500'} mt-1`}>Delivered</span>
...
function Spacer() {
    return <div className="w-full h-[1px] flex flex-row justify-between border border-gray-300 border-t-1" />
}